...
function Header({
  ...
  return (
    <header className="site-header">
      ...
      <div className="mood-switch" aria-label="Chọn mood">
        {themeBundle.themes
          .slice()
          .reverse()
          .map((theme) => (
        ...
        {FEATURED_TOKENS.map(([name, category, note]) => (
          <div className="token-row" key={name}>
            <span
              ...
              style={{ background: theme.semantic[name] }}
            />
            <span>
              <strong>{name}</strong>
  ...
  return (
    <section className="hero" id="overview">
      ...
      <div className="hero-copy">
        <h1>Một hệ màu,<br />ba nhịp Huế</h1>
        <p>
          Design tokens cho giao diện, mã nguồn
          <br />và những giờ làm việc thật dài.
        </p>
  ...
  return (
    <section className="syntax-panel" id="syntax">
      <div className="panel-heading">
        <h2>Syntax specimen</h2>
        <span>TypeScript⌄</span>
      </div>
      <pre aria-label="TypeScript syntax preview">
        <code>
    ...
    <section className="audit-panel" id="accessibility">
      ...
      <div className="audit-table">
        <div className="audit-head">
          <span>Token</span><span>Preview</span><span>Ratio</span><span>Result</span>
        </div>
        {rows.map(({ token, minimum, ratio, pass }) => (
          <div className="audit-row" key={token}>
            ...
            <span
              ...
              style={{ color: theme.semantic[token] }}
            >
              Aa
            </span>
            <span>{ratio.toFixed(2)}:1 <small>≥ {minimum}:1</small></span>
