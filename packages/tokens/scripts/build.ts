...
const json = `${JSON.stringify({ version: "0.1.0", themes }, null, 2)}\n`;
const typescript = `// Generated by scripts/build.ts. Do not edit.
export const themeBundle = ${JSON.stringify({ version: "0.1.0", themes }, null, 2)} as const;
...
for (const [name, content] of [
  ["themes.json", json],
  ["themes.ts", typescript],
  ["themes.css", css],
] as const) {
  const path = resolve(outputDirectory, name);
  if (checkOnly) {
    const existing = await readFile(path, "utf8").catch(() => "");
    if (existing !== content) throw new Error(`${name} is stale; run bun run build`);
  } else {
